...
AddEventHandler("bixbi_zipties:startZiptie", function(targetId)
	...
	-- if (Player(targetId).state.handsup ~= nil and Player(targetId).state.handsup) then
		...
		ESX.ShowNotification("You have ziptied the target.", "error", 3000)
	else
		ESX.ShowNotification("This person dosen't have their hands up.", "error", 3000)
	end
...
AddEventHandler("bixbi_zipties:startziptieremove", function(tool, targetId)
	if (targetId == nil) then
		...
		if closestPlayer ~= -1 and closestDistance <= 2.0 then 
			...
			ESX.ShowNotification("There isn\'t anyone nearby", "error", 3000)
			return
		end
	end
	Citizen.Wait(tool.timer * 1000)
	
	ESX.ShowNotification("You have removed the ziptie.", "error", 3000)
...
function StartHandcuffTimer()
	...
	handcuffTimer.task = ESX.SetTimeout(Config.HandcuffTimer, function()
		ESX.ShowNotification("You feel the zipties slowly losing grip and fading away.", "error", 3000)
...
AddEventHandler("bixbi_zipties:ziptie", function(source, tool)
	...
	ESX.ShowNotification("You have been ziptied.", "error", 3000)

	if (not LocalPlayer.state.ziptied) then
        LocalPlayer.state:set('ziptied', true, true)
		playAnim(playerPed, 're@stag_do@idle_a', 'idle_a_ped', -1)
...
AddEventHandler('bixbi_zipties:removeziptie', function()
	local playerPed = PlayerPedId()
	ESX.ShowNotification("You are free again.", "error", 3000)
