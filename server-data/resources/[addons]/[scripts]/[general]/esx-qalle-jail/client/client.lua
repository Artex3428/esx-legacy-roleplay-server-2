...
function OpenJailMenu()
	local elements = {
		{unselectable = true, icon = "fas fa-id-badge", title = "Jail menu"},
		{icon = "fas fa-address-card", title = "Jail closest person", value = 'jailperson'},
		{icon = "fas fa-times", title = "Unjail person", uniform = 'unjailperson'},
	}
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		elseif action == "unjailperson" then
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				local elements2 = {
					{unselectable = true, icon = "fas fa-times", title = "Unjail person"},
				}

				for i = 1, #playerArray, 1 do
					table.insert(elements2, {label = "Prisoner: " .. playerArray[i].name .. " | Jail Time: " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
				end
				...
				ESX.OpenContext("right", elements2, function(menu2,element2)
					...
					TriggerServerEvent("esx-qalle-jail:unJailPlayer", action2)
				end)
			end)
		end
	end)
