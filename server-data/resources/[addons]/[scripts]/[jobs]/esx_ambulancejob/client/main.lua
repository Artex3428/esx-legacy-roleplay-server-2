...
function RemoveItemsAfterRPDeath()
  ...
  CreateThread(function()
    ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
      ...
      -- Fade out the screen before respawning
      DoScreenFadeOut(800)

      -- Ensure screen fade-out happens before proceeding
      while not IsScreenFadedOut() do
        Wait(0)
      end

      -- Respawn the player at the closest hospital
      RespawnPed(PlayerPedId(), ClosestHospital.coords, ClosestHospital.heading)

      -- Wait for the game to process the respawn
      Wait(1000)

      -- Ensure the screen fades back in
      DoScreenFadeIn(800)

      -- Reset all death effects
      isDead = false
      ClearTimecycleModifier()
