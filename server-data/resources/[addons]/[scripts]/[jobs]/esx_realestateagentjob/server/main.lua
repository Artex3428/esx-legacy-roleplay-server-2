RegisterNetEvent('realestateagent-item:getItem', function(source, item)
    ...
    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = "REALESTATEAGENT", content = "```" ..fullname.. " ["..id.."] | has taken out "..item.." | "..DATE.."```"}), { ['Content-Type'] = 'application/json' })
end)
