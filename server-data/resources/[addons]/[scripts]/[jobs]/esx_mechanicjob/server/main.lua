RegisterNetEvent('mechanic-item:getItem', function(source, item)
    ...
    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = "MECHANIC", content = "```" ..fullname.. " ["..id.."] | has taken out "..item.." | "..DATE.."```"}), { ['Content-Type'] = 'application/json' })
end)
...
AddEventHandler('esx_mechanicjob:startHarvest4', function()
	local source = source
	PlayersHarvesting3[source] = true
	TriggerClientEvent('esx:showNotification', source, "Ramp retrival")
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.addInventoryItem('ramp', 1)
end)
...
AddEventHandler('esx_mechanicjob:startHarvest5', function()
	local source = source
	PlayersHarvesting3[source] = true
	TriggerClientEvent('esx:showNotification', source, "Rope retrival")
	local xPlayer = ESX.GetPlayerFromId(source)
