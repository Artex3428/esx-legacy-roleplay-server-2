...
RegisterKeyMapping('hud', 'Open Menu', 'keyboard', Config.OpenMenu)
...
RegisterNUICallback('cinematicMode', function(data, cb)
    ...
    if data.checked then
        ...
        if (IsPedInAnyVehicle(player) and not IsThisModelABicycle(vehicle)) then -- or not Menu.isOutMapChecked
            DisplayRadar(true)
        end
    end
    TriggerEvent("hud:client:playHudChecklistSound")
        ...
        if ESX.PlayerLoaded then
           ...
           -- local inVehicle = IsPedInAnyVehicle(ped, false)
            if inVehicle then
                ...
                -- Here is the reson why the minimap border is not shown while on foot // Maybe? artex
                ...
                DisplayRadar(false) -- not Menu.isOutMapChecked
            end
        else
...
CreateThread(function()
    while true do
        SetRadarBigmapEnabled(false, false)
        SetRadarZoom(1000) -- 1100 is for map texture the one before was 1000
        Wait(500)
    end
end)
