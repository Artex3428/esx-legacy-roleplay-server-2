...
local function noclipThread()
    while noclip do
        ...
        SetEntityCoordsNoOffset(PlayerPedId(), x, y, z, 0, 0, 0)
    ...
    SetFollowPedCamViewMode(previous_cam_view)
end
...
AddEventHandler("esx:noclip", function()
    ESX.TriggerServerCallback("esx:isUserAdmin", function(admin)
        ...
        noclip = not noclip
        if noclip then
            previous_cam_view = GetFollowPedCamViewMode()
            heading = GetEntityHeading(PlayerPedId())
